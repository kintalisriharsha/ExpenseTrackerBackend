...
otp_email.py
────────────
OTP email: HTML/plain template builders + async SMTP sender.

Threading notes
───────────────
_build_otp_html()   — pure string formatting; fast, stays sync.
_build_otp_plain()  — same.
send_otp_email()    — fully async via aiosmtplib (no OS thread needed).
                      SMTP I/O is handled by asyncio, NOT threads.

Why no threading here?
  aiosmtplib is a native asyncio SMTP client. It issues non-blocking
  socket operations directly on the event loop — equivalent to
  running SMTP in a thread pool, but without the thread overhead or
  GIL contention.  Adding run_in_executor on top would be redundant.
"""
...
def _build_otp_html(otp: str) -> str:
    """
    Builds a styled HTML email with one <td> per OTP digit.
    Pure string formatting — no I/O, stays synchronous.
    """
    digits = "".join(
        f'<td style="'
...
async def send_otp_email(to_email: str, otp: str) -> None:
    """
    Send an OTP email via SMTP using aiosmtplib (native asyncio).

    aiosmtplib issues all socket I/O on the event loop directly —
    no OS thread is needed.  The function is non-blocking from the
    caller's perspective.

    Raises:
        RuntimeError   — SMTP credentials not configured.
        aiosmtplib.*   — network / auth failures (caller should catch).
    """
    ...
    # Plain first, HTML last — clients prefer the last matching part
    ...
    try:
        await aiosmtplib.send(
            ...
            start_tls = True,   # STARTTLS on port 587 (Gmail requirement)
        )
        logger.warning(f"OTP email sent to {to_email}")
    except Exception as exc:
        logger.error(f"SMTP failure for {to_email}: {exc}")
        raise